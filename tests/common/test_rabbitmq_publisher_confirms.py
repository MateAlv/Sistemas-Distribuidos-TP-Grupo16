...
def test_queue_send_disables_mandatory_without_confirms(pika_env, monkeypatch):
    monkeypatch.delenv("RABBITMQ_PUBLISHER_CONFIRMS", raising=False)
    channel = pika_env.FakeChannel()
    connection = pika_env.FakeConnection(channel)
    ...
    assert channel.basic_publish_calls[-1][1]["mandatory"] is False


def test_queue_send_enables_mandatory_with_confirms(pika_env, monkeypatch):
    monkeypatch.setenv("RABBITMQ_PUBLISHER_CONFIRMS", "true")
    channel = pika_env.FakeChannel()
    connection = pika_env.FakeConnection(channel)
...
def test_exchange_send_disables_mandatory_without_confirms(pika_env, monkeypatch):
    monkeypatch.delenv("RABBITMQ_PUBLISHER_CONFIRMS", raising=False)
    channel = pika_env.FakeChannel()
    connection = pika_env.FakeConnection(channel)
...
def test_exchange_send_enables_mandatory_with_confirms(pika_env, monkeypatch):
    monkeypatch.setenv("RABBITMQ_PUBLISHER_CONFIRMS", "true")
    channel = pika_env.FakeChannel()
    connection = pika_env.FakeConnection(channel)
